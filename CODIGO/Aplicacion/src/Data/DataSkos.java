 ...
 * @author Smart
 */
public class DataSkos {
      private Categorias document = new Categorias  ();
    ...
    public ResultSet Consultarsujeto(Categorias  db) throws ClassNotFoundException, SQLException {
        Statement st = c.Conectar().getConexion().createStatement();//obj para poder manipular abre coneccion y crea unStatement 
        String Sentencia = "Select distinct sujeto from skos WHERE objeto="+db.getObjeto();//crea un a varible y ponermos la cadena para recuperar todo desde la BDD
        ResultSet rs = st.executeQuery(Sentencia);//recuper un un ResultSet y envio la varible a executeQuery
        return rs;//retornma en un obj ResultSet rs
    }
}
